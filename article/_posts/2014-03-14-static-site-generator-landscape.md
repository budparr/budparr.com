...
Generally speaking templates and layout schemes tend to be similar in both types of sites in that they use a central default or base layout, which may be overridden for various types of pages and enhanced with snippets of code that can be included in any template. While this layout scheme is not unique to no-db websites, it's a natural fit. In contrast to some of the popular, "traditional" content management systems, these apps assume nothing about your design, thankfully.
...
As of this writing, when used with GitHub Pages, Jekyll doesn't allow any plugins because of restrictions on the hosting platform. If deployment isn't an issue there are plugins put out there to overcome some of Jekyll’s--born a “blogging platform for hackers”--lacunae, and there will be a whitelist of plugins allowed in a future release.

Jekyll is also a bit more structured than many of the apps, using a post and page model (its original tagline was "blogging for hackers") where posts get taxonomies and chronological sorting whereas pages do not. The lack of taxonomies and sorting for pages is a big shortcoming, but I know that the project owners are working toward a more flexible model. All in all, I've enjoyed having to find creative solutions to Jekyll's constraints and have managed to create some fairly complex relationships between pieces of content, even if most everything has to be nested in little _posts folders scattered throughout the site.
